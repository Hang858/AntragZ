	...
	const int8_t *f, const int8_t *g, unsigned depth, uint32_t *tmp)
{
	...
	FFT((poly*)rt3, logn);
	FFT((poly*)rt4, logn);
	poly_invnorm2_fft((poly*)rt5, (poly*)rt3, (poly*)rt4, logn);
	poly_adj_fft((poly*)rt3, logn);
	poly_adj_fft((poly*)rt4, logn);
	FGlen = llen;
	...
	for (;;) {
		...
		FFT((poly*)rt1, logn);
		FFT((poly*)rt2, logn);
		poly_mul_fft((poly*)rt1, (poly*)rt3, logn);
		poly_mul_fft((poly*)rt2, (poly*)rt4, logn);
		poly_add((poly*)rt2, (poly*)rt1, logn);
		poly_mul_autoadj_fft((poly*)rt2, (poly*)rt5, logn);
		invFFT((poly*)rt2, logn);
...
int antrag_solve_ntru(int8_t *F, int8_t *G, const int8_t *f, const int8_t *g, unsigned logn, uint32_t *tmp) {
    ...
    if (!solve_NTRU_deepest(logn, f, g, tmp)) return 0;

    unsigned depth = logn;
    while (depth -- > 0) {
        if (!solve_NTRU_intermediate(logn, f, g, depth, tmp)) return 0;
    }

    if (!poly_big_to_small(F, tmp, 127, logn) || !poly_big_to_small(G, tmp + MKN(logn), 127, logn)) {
        return 0;
    }
    return 1;
}